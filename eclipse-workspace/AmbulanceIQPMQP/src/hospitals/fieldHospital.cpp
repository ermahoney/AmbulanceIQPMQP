...
std::vector<std::string> fieldHospital::top3FieldHospitalsCalculator(LinkedList* fieldHospitals, LinkedList* factors){
    ...
    LinkedList::LLNODE *pnode = fieldHospitals->get_first_node(reinterpret_cast<LinkedList::LLIST *>(fieldHospitals));
    ...
    if (frst == scnd && scnd == thrd && frst !=  std::numeric_limits<int>::max()) { //case for first, second, and third tied
    ...
    } else if (frst == scnd && frst != std::numeric_limits<int>::max() && thrd == std::numeric_limits<int>::max()) { //case for first and second tied
    ...
    } else if (scnd == thrd && scnd != std::numeric_limits<int>::max() && frst != std::numeric_limits<int>::max()) { //case for second and third tied
    ...
    } else if (thrd == std::numeric_limits<int>::max() && scnd != std::numeric_limits<int>::max()) { //case for 2 field hospitals provided
    ...
    } else if (scnd == std::numeric_limits<int>::max() && thrd == std::numeric_limits<int>::max()) { //case for 1 field hospital provided
...
double fieldHospital::routeDistanceScore(pinpoint fHLocation, pinpoint poi) {
    return eDistance(fHLocation, poi);
}
